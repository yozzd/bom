...
import { UpdateSupplier } from '../../apollo/supplier/mutation';
        ...
        if (valid) {
          try {
            ...
            await this.$apollo.mutate({
              ...
              variables: {
                input: {
                  suplierID: parseInt(this.form.suplierID, 10),
              ...
              update: (store, { data: { updateSupplier } }) => {
                const cdata = store.readQuery({
                  query: GetAllSupplier,
                });

                const index = cdata.getAllSupplier.findIndex(
                  (e) => e.suplierID === this.form.suplierID,
                );
                cdata.getAllSupplier[index] = updateSupplier;
                this.$emit('update', { value: cdata.getAllSupplier[index], index });
